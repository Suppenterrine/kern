...
use crate::reduction::{letters, numbers};
...
/// Lädt die eingebettete Bedeutungen-Liste.
...
/// Sucht den Text einer Zahl in der Bedeutungen-Liste.
...
/// Liefert den Reduktionspfad einer Eingabe.
pub fn reduce_number_steps(input: &str) -> (u32, Vec<String>) {
    ...
    let mut values = letters::to_ordinal(input);
    values.extend(
        input.chars().filter_map(|c| {
            if c.is_ascii_digit() {
                c.to_digit(10).map(|d| d as u32)
            } else {
                None
            }
        }),
    ...
    let mut num = numbers::sum(&values);
...
/// Führt eine Reduktion durch und gibt optional den Pfad aus.
...
/// Parst Range-Spezifikationen für Datumsberechnungen.
pub fn parse_range(spec: &str) -> Result<Vec<i32>, String> {
    ...
    // C) numerische Range:  -5..4   oder   3..-2
